...
Si vous avez bien un dossier ``.env`` ou ``.venv``, **attention, les commandes suivante sont à réaliser dans votre terminal en étant dans le dossier de votre projet:**
 - sous Windows  : vous devez faire ``.nom_du_dossier(venv ou env)\bin\activate``
 - sous Linux : vous devez faire source ``.nom_du_dossier(venv ou env)/bin/activate``
...
Souvent vous avez tendance à utiliser ``time.sleep()`` sauf qu'il s'agit d'une fonction qui bloque la totalité de votre programme.

Cependant, dans le module time il y a un moyen simple pour en faire un :
Les timestamp !
Pour récupérer une timestamp il suffit d'appeler la fonction time comme cela :
``time.time()``
...
Une méthode "normale" prend en paramètre l'instance de la classe appelant la méthode. ``f(self, *args)``
...
- **Le scripting système.** Python est un des très rare langage de programmation installé par la plupart des distributions Linux et en fait donc un outil intéressant pour toute personne se tournant vers l'administration système.
- **La programmation réseau.** Python a toujours été au devant de la scène lorsqu'il s'agissait d'attaquer des problèmes où la capacité à échanger efficacement des informations au sein d'un réseau primait sur l'efficacité propre de la machine. De nombreuses bibliothèques ont de tout temps été disponibles pour le réseau et le web comme asyncio, requests, beautifulsoup, tornado, twisted. Par exemple, OpenStack, le cloud open-source développé par RedHat est principalement développé en Python.

A contrario, Python **__n'est pas__** le langage que je recommanderais pour les sujets suivants:
...
**Opérations augmentant le compteur de références**
Les cas suivants augmentent le compteur de références de l'objet référencé par la variable b.
- l'affectation : ``a = b``
- l'appel de fonction, et tout autre appel à un objet *callable* : ``f(a)``
- la multiplication des listes (ou autre collection pouvant être multipliée) : ``[b] * n``
- les mot-clés return et yield : ``return b``

**Opérations décrémentant le compteur de références**
...
__Étape 1:__
Faire ``win`` puis chercher ``python`` puis cliquer sur ``désinstaller``, ça va ouvrir ``Programmes et fonctionnalité``

__Étape 2:__
Une fois dans ``Programmes et fonctionnalité`` chercher python  puis  ``clique droit``, ``modifier``

__Étape 3:__
 On se retrouve dans un menu avec différents choix ``Modify``,``repair``,``uninstall`` , choisir ``Modify`` puis ``next`` en bas à droite 
